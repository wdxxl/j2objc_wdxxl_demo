...
//  source: jar:file:lucene_fis-core-grouping-3.5.0-sources_arc.jar!org/apache/lucene/search/grouping/BlockGroupingCollector.java
//
...
- (OrgApacheLuceneSearchGroupingTopGroups *)getTopGroupsWithOrgApacheLuceneSearchSort:(OrgApacheLuceneSearchSort *)withinGroupSort
  ...
  OrgApacheLuceneSearchGroupingBlockGroupingCollector_FakeScorer *fakeScorer = new_OrgApacheLuceneSearchGroupingBlockGroupingCollector_FakeScorer_init();
  IOSObjectArray *groups = [IOSObjectArray newArrayWithLength:[groupQueue_ size] - groupOffset type:OrgApacheLuceneSearchGroupingGroupDocs_class_()];
  for (jint downTo = [groupQueue_ size] - groupOffset - 1; downTo >= 0; downTo--) {
    ...
    if (withinGroupSort == nil) {
      if (!needsScores_) {
        @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"cannot sort by relevance within group: needsScores=false");
      }
    ...
    if (fillSortFields) {
      groupSortValues = [IOSObjectArray newArrayWithLength:((IOSObjectArray *) nil_chk(comparators_))->size_ type:JavaLangComparable_class_()];
      for (jint sortFieldIDX = 0; sortFieldIDX < comparators_->size_; sortFieldIDX++) {
        (void) IOSObjectArray_Set(groupSortValues, sortFieldIDX, [((OrgApacheLuceneSearchFieldComparator *) nil_chk(IOSObjectArray_Get(comparators_, sortFieldIDX))) valueWithInt:og->comparatorSlot_]);
      }
    }
    else {
      groupSortValues = nil;
    ...
    (void) IOSObjectArray_SetAndConsume(groups, downTo, new_OrgApacheLuceneSearchGroupingGroupDocs_initWithFloat_withInt_withOrgApacheLuceneSearchScoreDocArray_withId_withNSObjectArray_([((OrgApacheLuceneSearchTopDocs *) nil_chk(topDocs)) getMaxScore], og->count_, topDocs->scoreDocs_, nil, groupSortValues));
  }
...
- (void)collectWithInt:(jint)doc {
  ...
  if (subDocUpto_ == ((IOSIntArray *) nil_chk(pendingSubDocs_))->size_) {
    pendingSubDocs_ = OrgApacheLuceneUtilArrayUtil_growWithIntArray_(pendingSubDocs_);
  }
  *IOSIntArray_GetRef(nil_chk(pendingSubDocs_), subDocUpto_) = doc;
  if (needsScores_) {
    if (subDocUpto_ == ((IOSFloatArray *) nil_chk(pendingSubScores_))->size_) {
      pendingSubScores_ = OrgApacheLuceneUtilArrayUtil_growWithFloatArray_(pendingSubScores_);
    }
...
- (void)setNextReaderWithOrgApacheLuceneIndexIndexReader:(OrgApacheLuceneIndexIndexReader *)reader
  ...
  lastDocPerGroupBits_ = [((OrgApacheLuceneSearchDocIdSet *) nil_chk([((OrgApacheLuceneSearchFilter *) nil_chk(lastDocPerGroup_)) getDocIdSetWithOrgApacheLuceneIndexIndexReader:reader])) iterator];
  groupEndDocID_ = -1;
...
void OrgApacheLuceneSearchGroupingBlockGroupingCollector_processGroup(OrgApacheLuceneSearchGroupingBlockGroupingCollector *self) {
  ...
  if (self->groupCompetes_) {
    if (!self->queueFull_) {
      OrgApacheLuceneSearchGroupingBlockGroupingCollector_OneGroup *og = new_OrgApacheLuceneSearchGroupingBlockGroupingCollector_OneGroup_init();
      og->count_ = self->subDocUpto_;
      og->topGroupDoc_ = self->docBase_ + self->topGroupDoc_;
      og->docs_ = self->pendingSubDocs_;
      self->pendingSubDocs_ = [IOSIntArray newArrayWithLength:10];
      if (self->needsScores_) {
        og->scores_ = self->pendingSubScores_;
        self->pendingSubScores_ = [IOSFloatArray newArrayWithLength:10];
      }
      og->reader_ = self->currentReader_;
      ...
      og->docs_ = self->pendingSubDocs_;
      self->pendingSubDocs_ = savDocs;
      if (self->needsScores_) {
        IOSFloatArray *savScores = og->scores_;
        og->scores_ = self->pendingSubScores_;
        self->pendingSubScores_ = savScores;
      }
      og->reader_ = self->currentReader_;
      og->docBase_ = self->docBase_;
...
void OrgApacheLuceneSearchGroupingBlockGroupingCollector_initWithOrgApacheLuceneSearchSort_withInt_withBoolean_withOrgApacheLuceneSearchFilter_(OrgApacheLuceneSearchGroupingBlockGroupingCollector *self, OrgApacheLuceneSearchSort *groupSort, jint topNGroups, jboolean needsScores, OrgApacheLuceneSearchFilter *lastDocPerGroup) {
  ...
  self->groupQueue_ = new_OrgApacheLuceneSearchGroupingBlockGroupingCollector_GroupQueue_initWithOrgApacheLuceneSearchGroupingBlockGroupingCollector_withInt_(self, topNGroups);
  self->pendingSubDocs_ = [IOSIntArray newArrayWithLength:10];
  if (needsScores) {
    self->pendingSubScores_ = [IOSFloatArray newArrayWithLength:10];
  }
  self->needsScores_ = needsScores;
  self->lastDocPerGroup_ = lastDocPerGroup;
  ...
  for (jint i = 0; i < sortFields->size_; i++) {
    ...
    (void) IOSObjectArray_Set(self->comparators_, i, [((OrgApacheLuceneSearchSortField *) nil_chk(sortField)) getComparatorWithInt:topNGroups withInt:i]);
