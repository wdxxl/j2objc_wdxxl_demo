...
//  source: jar:file:lucene_fis-core-grouping-3.5.0-sources_arc.jar!org/apache/lucene/index/SegmentNorms.java
//
...
@interface OrgApacheLuceneIndexSegmentNorms () {
 @public
  ...
  IOSByteArray *bytes_;
  jint number_;
  JavaLangRefWeakReference *owner_;
}
...
J2OBJC_FIELD_SETTER(OrgApacheLuceneIndexSegmentNorms, owner_, JavaLangRefWeakReference *)
...
- (void)incRef {
  @synchronized(self) {
    JreAssert(refCount_ > 0 && (origNorm_ == nil || origNorm_->refCount_ > 0), @"org/apache/lucene/index/SegmentNorms.java:68 condition failed: assert refCount > 0 && (origNorm == null || origNorm.refCount > 0);");
    refCount_++;
  }
}
...
- (void)decRef {
  @synchronized(self) {
    JreAssert(refCount_ > 0 && (origNorm_ == nil || origNorm_->refCount_ > 0), @"org/apache/lucene/index/SegmentNorms.java:91 condition failed: assert refCount > 0 && (origNorm == null || origNorm.refCount > 0);");
    if (--refCount_ == 0) {
      if (origNorm_ != nil) {
        [origNorm_ decRef];
        origNorm_ = nil;
      ...
      if (bytes_ != nil) {
        JreAssert(bytesRef_ != nil, @"org/apache/lucene/index/SegmentNorms.java:102 condition failed: assert bytesRef != null;");
        [((JavaUtilConcurrentAtomicAtomicInteger *) nil_chk(bytesRef_)) decrementAndGet];
        bytes_ = nil;
        bytesRef_ = nil;
      }
      else {
        JreAssert(bytesRef_ == nil, @"org/apache/lucene/index/SegmentNorms.java:107 condition failed: assert bytesRef == null;");
      }
    }
  }
}
...
- (void)bytesWithByteArray:(IOSByteArray *)bytesOut
                   ...
                   withInt:(jint)len {
  @synchronized(self) {
    JreAssert(refCount_ > 0 && (origNorm_ == nil || origNorm_->refCount_ > 0), @"org/apache/lucene/index/SegmentNorms.java:115 condition failed: assert refCount > 0 && (origNorm == null || origNorm.refCount > 0);");
    if (bytes_ != nil) {
      JreAssert(len <= [((OrgApacheLuceneIndexSegmentReader *) nil_chk([((JavaLangRefWeakReference *) nil_chk(owner_)) get])) maxDoc], @"org/apache/lucene/index/SegmentNorms.java:118 condition failed: assert len <= owner.get().maxDoc();");
...
- (IOSByteArray *)bytes {
  @synchronized(self) {
    JreAssert(refCount_ > 0 && (origNorm_ == nil || origNorm_->refCount_ > 0), @"org/apache/lucene/index/SegmentNorms.java:137 condition failed: assert refCount > 0 && (origNorm == null || origNorm.refCount > 0);");
    if (bytes_ == nil) {
      JreAssert(bytesRef_ == nil, @"org/apache/lucene/index/SegmentNorms.java:139 condition failed: assert bytesRef == null;");
      if (origNorm_ != nil) {
        ...
        jint count = [((OrgApacheLuceneIndexSegmentReader *) nil_chk([((JavaLangRefWeakReference *) nil_chk(owner_)) get])) maxDoc];
        bytes_ = [IOSByteArray newArrayWithLength:count];
        JreAssert(in_ != nil, @"org/apache/lucene/index/SegmentNorms.java:160 condition failed: assert in != null;");
        @synchronized(in_) {
          [((OrgApacheLuceneStoreIndexInput *) nil_chk(in_)) seekWithLong:normSeek_];
...
- (IOSByteArray *)copyOnWrite {
  @synchronized(self) {
    JreAssert(refCount_ > 0 && (origNorm_ == nil || origNorm_->refCount_ > 0), @"org/apache/lucene/index/SegmentNorms.java:184 condition failed: assert refCount > 0 && (origNorm == null || origNorm.refCount > 0);");
    (void) [self bytes];
    JreAssert(bytes_ != nil, @"org/apache/lucene/index/SegmentNorms.java:186 condition failed: assert bytes != null;");
    JreAssert(bytesRef_ != nil, @"org/apache/lucene/index/SegmentNorms.java:187 condition failed: assert bytesRef != null;");
    if ([((JavaUtilConcurrentAtomicAtomicInteger *) nil_chk(bytesRef_)) get] > 1) {
      JreAssert(refCount_ == 1, @"org/apache/lucene/index/SegmentNorms.java:192 condition failed: assert refCount == 1;");
      JavaUtilConcurrentAtomicAtomicInteger *oldRef = bytesRef_;
      bytes_ = [((OrgApacheLuceneIndexSegmentReader *) nil_chk([((JavaLangRefWeakReference *) nil_chk(owner_)) get])) cloneNormBytesWithByteArray:bytes_];
      bytesRef_ = new_JavaUtilConcurrentAtomicAtomicInteger_initWithInt_(1);
...
- (id)java_clone {
  @synchronized(self) {
    JreAssert(refCount_ > 0 && (origNorm_ == nil || origNorm_->refCount_ > 0), @"org/apache/lucene/index/SegmentNorms.java:206 condition failed: assert refCount > 0 && (origNorm == null || origNorm.refCount > 0);");
    ...
    if (bytes_ != nil) {
      JreAssert(bytesRef_ != nil, @"org/apache/lucene/index/SegmentNorms.java:218 condition failed: assert bytesRef != null;");
      JreAssert(origNorm_ == nil, @"org/apache/lucene/index/SegmentNorms.java:219 condition failed: assert origNorm == null;");
      [((JavaUtilConcurrentAtomicAtomicInteger *) nil_chk(clone->bytesRef_)) incrementAndGet];
    }
    else {
      JreAssert(bytesRef_ == nil, @"org/apache/lucene/index/SegmentNorms.java:224 condition failed: assert bytesRef == null;");
      if (origNorm_ == nil) {
...
- (void)reWriteWithOrgApacheLuceneIndexSegmentInfo:(OrgApacheLuceneIndexSegmentInfo *)si {
  ...
  OrgApacheLuceneStoreIndexOutput *out = [((OrgApacheLuceneStoreDirectory *) nil_chk([((OrgApacheLuceneIndexSegmentReader *) nil_chk([((JavaLangRefWeakReference *) nil_chk(owner_)) get])) directory])) createOutputWithNSString:normFileName];
  ...
  @try {
    @try {
      ...
      [out writeBytesWithByteArray:bytes_ withInt:[((OrgApacheLuceneIndexSegmentReader *) nil_chk([owner_ get])) maxDoc]];
    }
    @finally {
      [out close];
    }
  ...
  @finally {
    if (!success) {
      @try {
        [((OrgApacheLuceneStoreDirectory *) nil_chk([((OrgApacheLuceneIndexSegmentReader *) nil_chk([owner_ get])) directory])) deleteFileWithNSString:normFileName];
      }
...
+ (const J2ObjcClassInfo *)__metadata {
  ...
  static const J2ObjcFieldInfo fields[] = {
    ...
    { "owner_", "LJavaLangRefWeakReference;", .constantValue.asLong = 0, 0x12, -1, -1, 8, -1 },
  };
  static const void *ptrTable[] = { "LOrgApacheLuceneStoreIndexInput;IJLOrgApacheLuceneIndexSegmentReader;", "LJavaIoIOException;", "bytes", "[BII", "clone", "reWrite", "LOrgApacheLuceneIndexSegmentInfo;", &OrgApacheLuceneIndexSegmentNorms_NORMS_HEADER, "Ljava/lang/ref/WeakReference<Lorg/apache/lucene/index/SegmentReader;>;" };
...
void OrgApacheLuceneIndexSegmentNorms_initWithOrgApacheLuceneStoreIndexInput_withInt_withLong_withOrgApacheLuceneIndexSegmentReader_(OrgApacheLuceneIndexSegmentNorms *self, OrgApacheLuceneStoreIndexInput *inArg, jint number, jlong normSeek, OrgApacheLuceneIndexSegmentReader *owner) {
  ...
  self->owner_ = new_JavaLangRefWeakReference_initWithId_(owner);
}
...
void OrgApacheLuceneIndexSegmentNorms_closeInput(OrgApacheLuceneIndexSegmentNorms *self) {
  if (self->in_ != nil) {
    if (self->in_ != ((OrgApacheLuceneIndexSegmentReader *) nil_chk([((JavaLangRefWeakReference *) nil_chk(self->owner_)) get]))->singleNormStream_) {
      [((OrgApacheLuceneStoreIndexInput *) nil_chk(self->in_)) close];
    }
    else {
      if ([((JavaUtilConcurrentAtomicAtomicInteger *) nil_chk(((OrgApacheLuceneIndexSegmentReader *) nil_chk([self->owner_ get]))->singleNormRef_)) decrementAndGet] == 0) {
        [((OrgApacheLuceneStoreIndexInput *) nil_chk(((OrgApacheLuceneIndexSegmentReader *) nil_chk([self->owner_ get]))->singleNormStream_)) close];
        ((OrgApacheLuceneIndexSegmentReader *) nil_chk([self->owner_ get]))->singleNormStream_ = nil;
      }
    }
    self->in_ = nil;
  }
