...
+ (void)initialize {
  if (self == [OrgApacheCommonsLang3ReflectMemberUtils class]) {
    OrgApacheCommonsLang3ReflectMemberUtils_ORDERED_PRIMITIVE_TYPES = [IOSObjectArray newArrayWithObjects:(id[]){ JreLoadStatic(JavaLangByte, TYPE), JreLoadStatic(JavaLangShort, TYPE), JreLoadStatic(JavaLangCharacter, TYPE), JreLoadStatic(JavaLangInteger, TYPE), JreLoadStatic(JavaLangLong, TYPE), JreLoadStatic(JavaLangFloat, TYPE), JreLoadStatic(JavaLangDouble, TYPE) } count:7 type:IOSClass_class_()];
