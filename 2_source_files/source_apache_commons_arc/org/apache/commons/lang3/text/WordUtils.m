...
NSString *OrgApacheCommonsLang3TextWordUtils_wrapWithNSString_withInt_withNSString_withBoolean_(NSString *str, jint wrapLength, NSString *newLineStr, jboolean wrapLongWords) {
  ...
  jint offset = 0;
  JavaLangStringBuilder *wrappedLine = new_JavaLangStringBuilder_initWithInt_(inputLineLength + 32);
  while (offset < inputLineLength) {
    ...
    if (spaceToWrapAt >= offset) {
      (void) [wrappedLine appendWithNSString:[str java_substring:offset endIndex:spaceToWrapAt]];
      (void) [wrappedLine appendWithNSString:newLineStr];
      offset = spaceToWrapAt + 1;
    }
    else {
      if (wrapLongWords) {
        (void) [wrappedLine appendWithNSString:[str java_substring:offset endIndex:wrapLength + offset]];
        (void) [wrappedLine appendWithNSString:newLineStr];
        offset += wrapLength;
      }
      else {
        ...
        if (spaceToWrapAt >= 0) {
          ...
          (void) [wrappedLine appendWithNSString:[str java_substring:offset]];
...
NSString *OrgApacheCommonsLang3TextWordUtils_initialsWithNSString_withCharArray_(NSString *str, IOSCharArray *delimiters) {
  ...
  IOSCharArray *buf = [IOSCharArray newArrayWithLength:strLen / 2 + 1];
  jint count = 0;
