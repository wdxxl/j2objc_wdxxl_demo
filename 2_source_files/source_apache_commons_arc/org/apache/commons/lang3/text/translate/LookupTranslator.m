...
void OrgApacheCommonsLang3TextTranslateLookupTranslator_initWithJavaLangCharSequenceArray2_(OrgApacheCommonsLang3TextTranslateLookupTranslator *self, IOSObjectArray *lookup) {
  ...
  self->lookupMap_ = new_JavaUtilHashMap_init();
  self->prefixSet_ = new_JavaUtilHashSet_init();
  ...
  if (lookup != nil) {
    {
      ...
      while (b__ < e__) {
        ...
        (void) [self->lookupMap_ putWithId:[((id<JavaLangCharSequence>) nil_chk(IOSObjectArray_Get(nil_chk(seq), 0))) description] withId:[((id<JavaLangCharSequence>) nil_chk(IOSObjectArray_Get(seq, 1))) description]];
