  ...
  if ([((id<JavaLangCharSequence>) nil_chk(input)) charAtWithInt:index] == '\\' && index + 1 < [input java_length] && [input charAtWithInt:index + 1] == 'u') {
    ...
    if (index + i + 4 <= [input java_length]) {
      ...
      @catch (JavaLangNumberFormatException *nfe) {
        @throw new_JavaLangIllegalArgumentException_initWithNSString_withNSException_(JreStrcat("$@", @"Unable to parse unicode value: ", unicode), nfe);
      }
      return i + 4;
    }
    @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$@$", @"Less than 4 hex digits in unicode value: '", [input subSequenceFrom:index to:[input java_length]], @"' due to end of CharSequence"));
  }
  return 0;
}
