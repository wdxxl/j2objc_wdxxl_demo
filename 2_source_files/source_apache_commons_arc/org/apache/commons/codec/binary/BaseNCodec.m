...
- (IOSByteArray *)decodeWithByteArray:(IOSByteArray *)pArray {
  ...
  OrgApacheCommonsCodecBinaryBaseNCodec_Context *context = new_OrgApacheCommonsCodecBinaryBaseNCodec_Context_init();
  ...
  IOSByteArray *result = [IOSByteArray newArrayWithLength:context->pos_];
...
- (IOSByteArray *)encodeWithByteArray:(IOSByteArray *)pArray {
  ...
  IOSByteArray *buf = [IOSByteArray newArrayWithLength:context->pos_ - context->readPos_];
...
IOSByteArray *OrgApacheCommonsCodecBinaryBaseNCodec_resizeBufferWithOrgApacheCommonsCodecBinaryBaseNCodec_Context_(OrgApacheCommonsCodecBinaryBaseNCodec *self, OrgApacheCommonsCodecBinaryBaseNCodec_Context *context) {
  if (((OrgApacheCommonsCodecBinaryBaseNCodec_Context *) nil_chk(context))->buffer_ == nil) {
    context->buffer_ = [IOSByteArray newArrayWithLength:[self getDefaultBufferSize]];
    context->pos_ = 0;
    context->readPos_ = 0;
  }
  else {
    IOSByteArray *b = [IOSByteArray newArrayWithLength:context->buffer_->size_ * OrgApacheCommonsCodecBinaryBaseNCodec_DEFAULT_BUFFER_RESIZE_FACTOR];
...
- (NSString *)description {
  return NSString_java_formatWithNSString_withNSObjectArray_(@"%s[buffer=%s, currentLinePos=%s, eof=%s, ibitWorkArea=%s, lbitWorkArea=%s, modulus=%s, pos=%s, readPos=%s]", [IOSObjectArray newArrayWithObjects:(id[]){ [[self java_getClass] getSimpleName], JavaUtilArrays_toStringWithByteArray_(buffer_), JavaLangInteger_valueOfWithInt_(currentLinePos_), JavaLangBoolean_valueOfWithBoolean_(eof_), JavaLangInteger_valueOfWithInt_(ibitWorkArea_), JavaLangLong_valueOfWithLong_(lbitWorkArea_), JavaLangInteger_valueOfWithInt_(modulus_), JavaLangInteger_valueOfWithInt_(pos_), JavaLangInteger_valueOfWithInt_(readPos_) } count:9 type:NSObject_class_()]);
}
