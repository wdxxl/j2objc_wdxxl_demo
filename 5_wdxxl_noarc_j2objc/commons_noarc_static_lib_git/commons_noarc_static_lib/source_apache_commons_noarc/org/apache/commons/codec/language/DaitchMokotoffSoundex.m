...
- (id)encodeWithId:(id)obj {
  if (!([obj isKindOfClass:[NSString class]])) {
    @throw create_OrgApacheCommonsCodecEncoderException_initWithNSString_(@"Parameter supplied to DaitchMokotoffSoundex encode is not of type java.lang.String");
  }
...
+ (void)initialize {
  if (self == [OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex class]) {
    JreStrongAssignAndConsume(&OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_RULES, new_JavaUtilHashMap_init());
    JreStrongAssignAndConsume(&OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_FOLDINGS, new_JavaUtilHashMap_init());
    {
      ...
      if (rulesIS == nil) {
        @throw create_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$$", @"Unable to load resource: ", OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_RESOURCE_FILE));
      }
      JavaUtilScanner *scanner = create_JavaUtilScanner_initWithJavaIoInputStream_withNSString_(rulesIS, OrgApacheCommonsCodecCharEncoding_UTF_8);
      ...
      for (id<JavaUtilMap_Entry> __strong rule in nil_chk([OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_RULES entrySet])) {
        ...
        JavaUtilCollections_sortWithJavaUtilList_withJavaUtilComparator_(ruleList, create_OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_1_init());
      }
    }
    ...
    if ([((NSString *) nil_chk(line)) java_hasPrefix:OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_MULTILINE_COMMENT_START]) {
      ...
      if ([line java_contains:@"="]) {
        ...
        if (((IOSObjectArray *) nil_chk(parts))->size_ != 2) {
          @throw create_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$I$$$$", @"Malformed folding statement split into ", parts->size_, @" parts: ", rawLine, @" in ", location));
        }
        else {
          ...
          if ([((NSString *) nil_chk(leftCharacter)) java_length] != 1 || [((NSString *) nil_chk(rightCharacter)) java_length] != 1) {
            @throw create_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$$$$", @"Malformed folding statement - patterns are not single characters: ", rawLine, @" in ", location));
          }
          [((id<JavaUtilMap>) nil_chk(asciiFoldings)) putWithId:JavaLangCharacter_valueOfWithChar_([leftCharacter charAtWithInt:0]) withId:JavaLangCharacter_valueOfWithChar_([((NSString *) nil_chk(rightCharacter)) charAtWithInt:0])];
        }
      }
      else {
        ...
        if (((IOSObjectArray *) nil_chk(parts))->size_ != 4) {
          ...
          @try {
            ...
            OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_Rule *r = create_OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_Rule_initWithNSString_withNSString_withNSString_withNSString_(pattern, replacement1, replacement2, replacement3);
            ...
            if (rules == nil) {
              ...
              [ruleMapping putWithId:JavaLangCharacter_valueOfWithChar_(patternKey) withId:rules];
            }
            [rules addWithId:r];
          }
...
IOSObjectArray *OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_soundexWithNSString_withBoolean_(OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex *self, NSString *source, jboolean branching) {
  ...
  id<JavaUtilSet> currentBranches = create_JavaUtilLinkedHashSet_init();
  [currentBranches addWithId:create_OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_Branch_init()];
  ...
  for (jint index = 0; index < [((NSString *) nil_chk(input)) java_length]; index++) {
    ...
    id<JavaUtilList> nextBranches = branching ? create_JavaUtilArrayList_init() : JreLoadStatic(JavaUtilCollections, EMPTY_LIST);
  ...
  IOSObjectArray *result = [IOSObjectArray arrayWithLength:[currentBranches size] type:NSString_class_()];
  jint index = 0;
  for (OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_Branch * __strong branch in currentBranches) {
    [((OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_Branch *) nil_chk(branch)) finish];
    IOSObjectArray_Set(result, index++, [branch description]);
  }
  return result;
}
...
- (OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_Branch *)createBranch {
  OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_Branch *branch = create_OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_Branch_init();
  [((JavaLangStringBuilder *) nil_chk(branch->builder_)) appendWithNSString:[self description]];
  JreStrongAssign(&branch->lastReplacement_, self->lastReplacement_);
  return branch;
}
...
- (void)finish {
  while ([((JavaLangStringBuilder *) nil_chk(builder_)) java_length] < OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_MAX_LENGTH) {
    [builder_ appendWithChar:'0'];
    JreStrongAssign(&cachedString_, nil);
  }
}
...
- (void)processNextReplacementWithNSString:(NSString *)replacement
  ...
  if (append && [((JavaLangStringBuilder *) nil_chk(builder_)) java_length] < OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_MAX_LENGTH) {
    [((JavaLangStringBuilder *) nil_chk(builder_)) appendWithNSString:replacement];
    if ([builder_ java_length] > OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_MAX_LENGTH) {
      [builder_ delete__WithInt:OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_MAX_LENGTH withInt:[builder_ java_length]];
    }
    JreStrongAssign(&cachedString_, nil);
  }
  JreStrongAssign(&lastReplacement_, replacement);
}

- (NSString *)description {
  if (cachedString_ == nil) {
    JreStrongAssign(&cachedString_, [((JavaLangStringBuilder *) nil_chk(builder_)) description]);
  }
...
- (void)dealloc {
  RELEASE_(builder_);
  RELEASE_(cachedString_);
  RELEASE_(lastReplacement_);
  [super dealloc];
}
...
void OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_Branch_init(OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_Branch *self) {
  ...
  JreStrongAssign(&self->lastReplacement_, nil);
  JreStrongAssign(&self->cachedString_, nil);
}
...
- (NSString *)description {
  return NSString_java_formatWithNSString_withNSObjectArray_(@"%s=(%s,%s,%s)", [IOSObjectArray arrayWithObjects:(id[]){ pattern_, JavaUtilArrays_asListWithNSObjectArray_(replacementAtStart_), JavaUtilArrays_asListWithNSObjectArray_(replacementBeforeVowel_), JavaUtilArrays_asListWithNSObjectArray_(replacementDefault_) } count:4 type:NSObject_class_()]);
}

- (void)dealloc {
  RELEASE_(pattern_);
  RELEASE_(replacementAtStart_);
  RELEASE_(replacementBeforeVowel_);
  RELEASE_(replacementDefault_);
  [super dealloc];
}
...
void OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_Rule_initWithNSString_withNSString_withNSString_withNSString_(OrgApacheCommonsCodecLanguageDaitchMokotoffSoundex_Rule *self, NSString *pattern, NSString *replacementAtStart, NSString *replacementBeforeVowel, NSString *replacementDefault) {
  ...
  JreStrongAssign(&self->replacementAtStart_, [((NSString *) nil_chk(replacementAtStart)) java_split:@"\\|"]);
  JreStrongAssign(&self->replacementBeforeVowel_, [((NSString *) nil_chk(replacementBeforeVowel)) java_split:@"\\|"]);
  JreStrongAssign(&self->replacementDefault_, [((NSString *) nil_chk(replacementDefault)) java_split:@"\\|"]);
}
