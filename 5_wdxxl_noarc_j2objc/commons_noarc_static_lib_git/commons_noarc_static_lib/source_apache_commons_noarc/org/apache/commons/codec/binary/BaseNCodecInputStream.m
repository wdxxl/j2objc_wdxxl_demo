  ...
  if (b == nil) {
    ...
    while (readLen == 0) {
      if (![((OrgApacheCommonsCodecBinaryBaseNCodec *) nil_chk(baseNCodec_)) hasDataWithOrgApacheCommonsCodecBinaryBaseNCodec_Context:context_]) {
        IOSByteArray *buf = [IOSByteArray arrayWithLength:doEncode_ ? 4096 : 8192];
...
- (void)reset {
  @synchronized(self) {
    @throw create_JavaIoIOException_initWithNSString_(@"mark/reset not supported");
  }
}

- (jlong)skipWithLong:(jlong)n {
  if (n < 0) {
    @throw create_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$J", @"Negative skip length: ", n));
  }
  IOSByteArray *b = [IOSByteArray arrayWithLength:512];
  jlong todo = n;
  while (todo > 0) {
...
void OrgApacheCommonsCodecBinaryBaseNCodecInputStream_initWithJavaIoInputStream_withOrgApacheCommonsCodecBinaryBaseNCodec_withBoolean_(OrgApacheCommonsCodecBinaryBaseNCodecInputStream *self, JavaIoInputStream *inArg, OrgApacheCommonsCodecBinaryBaseNCodec *baseNCodec, jboolean doEncode) {
  ...
  JreStrongAssign(&self->baseNCodec_, baseNCodec);
}
