...
- (id)encodeWithId:(id)obj {
  if (!([obj isKindOfClass:[NSString class]])) {
    @throw create_OrgApacheCommonsCodecEncoderException_initWithNSString_(@"Parameter supplied to Soundex encode is not of type java.lang.String");
  }
...
- (NSString *)soundexWithNSString:(NSString *)str {
  ...
  IOSCharArray *out = [IOSCharArray arrayWithChars:(jchar[]){ '0', '0', '0', '0' } count:4];
  jchar last;
  jchar mapped;
  jint incount = 1;
  jint count = 1;
...
+ (void)initialize {
  if (self == [OrgApacheCommonsCodecLanguageSoundex class]) {
    JreStrongAssign(&OrgApacheCommonsCodecLanguageSoundex_US_ENGLISH_MAPPING, [((NSString *) nil_chk(OrgApacheCommonsCodecLanguageSoundex_US_ENGLISH_MAPPING_STRING)) java_toCharArray]);
    JreStrongAssignAndConsume(&OrgApacheCommonsCodecLanguageSoundex_US_ENGLISH, new_OrgApacheCommonsCodecLanguageSoundex_init());
...
void OrgApacheCommonsCodecLanguageSoundex_init(OrgApacheCommonsCodecLanguageSoundex *self) {
  NSObject_init(self);
  self->maxLength_ = 4;
  JreStrongAssign(&self->soundexMapping_, OrgApacheCommonsCodecLanguageSoundex_US_ENGLISH_MAPPING);
}
...
jchar OrgApacheCommonsCodecLanguageSoundex_mapWithChar_(OrgApacheCommonsCodecLanguageSoundex *self, jchar ch) {
  ...
  if (index < 0 || index >= ((IOSCharArray *) nil_chk(OrgApacheCommonsCodecLanguageSoundex_getSoundexMapping(self)))->size_) {
    @throw create_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$C", @"The character is not mapped: ", ch));
  }
