...
+ (void)initialize {
  if (self == [OrgApacheCommonsCodecDigestSha2Crypt class]) {
    JreStrongAssign(&OrgApacheCommonsCodecDigestSha2Crypt_SALT_PATTERN, JavaUtilRegexPattern_compileWithNSString_(@"^\\$([56])\\$(rounds=(\\d+)\\$)?([\\.\\/a-zA-Z0-9]{1,16}).*"));
...
NSString *OrgApacheCommonsCodecDigestSha2Crypt_sha2CryptWithByteArray_withNSString_withNSString_withInt_withNSString_(IOSByteArray *keyBytes, NSString *salt, NSString *saltPrefix, jint blocksize, NSString *algorithm) {
  ...
  if (salt == nil) {
    @throw create_JavaLangIllegalArgumentException_initWithNSString_(@"Salt must not be null");
  }
  ...
  IOSByteArray *pBytes = [IOSByteArray arrayWithLength:keyLen];
  jint cp = 0;
  ...
  IOSByteArray *sBytes = [IOSByteArray arrayWithLength:saltLen];
  cp = 0;
  ...
  JavaLangStringBuilder *buffer = create_JavaLangStringBuilder_initWithNSString_(saltPrefix);
  if (roundsCustom) {
    [buffer appendWithNSString:OrgApacheCommonsCodecDigestSha2Crypt_ROUNDS_PREFIX];
    [buffer appendWithInt:rounds];
    [buffer appendWithNSString:@"$"];
  }
  [buffer appendWithNSString:saltString];
  [buffer appendWithNSString:@"$"];
  if (blocksize == 32) {
