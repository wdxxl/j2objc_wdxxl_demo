...
//  source: jar:file:lucene_fis-core-grouping-3.5.0-sources_arc.jar!org/apache/lucene/search/FieldComparator.java
//
...
void OrgApacheLuceneSearchFieldComparator_NumericComparator_setNextReaderWithOrgApacheLuceneIndexIndexReader_withInt_(OrgApacheLuceneSearchFieldComparator_NumericComparator *self, OrgApacheLuceneIndexIndexReader *reader, jint docBase) {
  if (self->missingValue_ != nil) {
    JreStrongAssign(&self->docsWithField_, [((id<OrgApacheLuceneSearchFieldCache>) nil_chk(JreLoadStatic(OrgApacheLuceneSearchFieldCache, DEFAULT))) getDocsWithFieldWithOrgApacheLuceneIndexIndexReader:reader withNSString:self->field_]);
    if ([self->docsWithField_ isKindOfClass:[OrgApacheLuceneUtilBits_MatchAllBits class]]) {
      JreStrongAssign(&self->docsWithField_, nil);
    }
  }
  else {
    JreStrongAssign(&self->docsWithField_, nil);
