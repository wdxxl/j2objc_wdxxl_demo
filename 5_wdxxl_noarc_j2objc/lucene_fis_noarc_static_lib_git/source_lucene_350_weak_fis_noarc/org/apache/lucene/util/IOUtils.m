...
//  source: jar:file:lucene_fis-core-grouping-3.5.0-sources_arc.jar!org/apache/lucene/util/IOUtils.java
//
...
+ (void)initialize {
  if (self == [OrgApacheLuceneUtilIOUtils class]) {
    JreStrongAssign(&OrgApacheLuceneUtilIOUtils_CHARSET_UTF_8, JavaNioCharsetCharset_forNameWithNSString_(@"UTF-8"));
    {
      ...
      @try {
        m = [JavaLangThrowable_class_() getMethod:@"addSuppressed" parameterTypes:[IOSObjectArray arrayWithObjects:(id[]){ JavaLangThrowable_class_() } count:1 type:IOSClass_class_()]];
      }
