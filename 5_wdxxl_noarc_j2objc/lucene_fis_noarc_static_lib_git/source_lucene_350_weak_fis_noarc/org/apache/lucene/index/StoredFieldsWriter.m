...
//  source: jar:file:lucene_fis-core-grouping-3.5.0-sources_arc.jar!org/apache/lucene/index/StoredFieldsWriter.java
//
  ...
  @synchronized(self) {
    if (freeCount_ == 0) {
      ...
      if (allocCount_ > ((IOSObjectArray *) nil_chk(docFreeList_))->size_) {
        JreAssert(allocCount_ == 1 + docFreeList_->size_, @"org/apache/lucene/index/StoredFieldsWriter.java:83 condition failed: assert allocCount == 1+docFreeList.length;");
      ...
      return JreRetainedLocalValue(create_OrgApacheLuceneIndexStoredFieldsWriter_PerDoc_initWithOrgApacheLuceneIndexStoredFieldsWriter_(self));
    }
    else {
      return JreRetainedLocalValue(IOSObjectArray_Get(nil_chk(docFreeList_), --freeCount_));
    }
  }
}
...
- (void)finishDocumentWithOrgApacheLuceneIndexStoredFieldsWriter_PerDoc:(OrgApacheLuceneIndexStoredFieldsWriter_PerDoc *)perDoc {
  @synchronized(self) {
    ...
    JreAssert([((OrgApacheLuceneIndexIndexWriter *) nil_chk(docWriter_->writer_)) testPointWithNSString:@"StoredFieldsWriter.finishDocument end"], @"org/apache/lucene/index/StoredFieldsWriter.java:121 condition failed: assert docWriter.writer.testPoint(\"StoredFieldsWriter.finishDocument end\");");
  }
}

- (void)freeWithOrgApacheLuceneIndexStoredFieldsWriter_PerDoc:(OrgApacheLuceneIndexStoredFieldsWriter_PerDoc *)perDoc {
  @synchronized(self) {
    JreAssert(freeCount_ < ((IOSObjectArray *) nil_chk(docFreeList_))->size_, @"org/apache/lucene/index/StoredFieldsWriter.java:125 condition failed: assert freeCount < docFreeList.length;");
    JreAssert(0 == ((OrgApacheLuceneIndexStoredFieldsWriter_PerDoc *) nil_chk(perDoc))->numStoredFields_, @"org/apache/lucene/index/StoredFieldsWriter.java:126 condition failed: assert 0 == perDoc.numStoredFields;");
    JreAssert(0 == [((OrgApacheLuceneStoreRAMOutputStream *) nil_chk(perDoc->fdt_)) length], @"org/apache/lucene/index/StoredFieldsWriter.java:127 condition failed: assert 0 == perDoc.fdt.length();");
    JreAssert(0 == [((OrgApacheLuceneStoreRAMOutputStream *) nil_chk(perDoc->fdt_)) getFilePointer], @"org/apache/lucene/index/StoredFieldsWriter.java:128 condition failed: assert 0 == perDoc.fdt.getFilePointer();");
    IOSObjectArray_Set(nil_chk(docFreeList_), freeCount_++, perDoc);
  }
}

- (void)__javaClone:(OrgApacheLuceneIndexStoredFieldsWriter *)original {
