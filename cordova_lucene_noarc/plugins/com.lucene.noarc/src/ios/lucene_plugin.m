...
- (void)coolMethod:(CDVInvokedUrlCommand*)command; //FIMFRM
- (void)coolMethodTSM:(CDVInvokedUrlCommand*)command; //TSM
...
- (void)coolMethod:(CDVInvokedUrlCommand*)command
{
    @autoreleasepool{
      ...
      NSString *appPath = [appFolderPath stringByAppendingPathComponent:@"www/extract/frmfim/"];
...
- (void)coolMethodTSM:(CDVInvokedUrlCommand*)command
{
    @autoreleasepool{
        CDVPluginResult* pluginResult = nil;
        NSString* echo = [command.arguments objectAtIndex:0];
        ...
        NSString *appFolderPath = [[NSBundle mainBundle] resourcePath];
        NSFileManager *fileManager = [NSFileManager defaultManager];
        NSString *appPath = [appFolderPath stringByAppendingPathComponent:@"www/extract/tsm/"];
        NSArray *unsortedDDLPath = [fileManager contentsOfDirectoryAtPath:appPath error:nil];
        NSArray *sortedStrings =[unsortedDDLPath sortedArrayUsingSelector:@selector(compare:)];
        for(id obj in sortedStrings) {
            NSString *fullObj = [appPath stringByAppendingPathComponent:obj];
        ...
        NSString *documentPath = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex: 0];
        NSString *_unzipPath = [documentPath stringByAppendingPathComponent:@"flatirons/extract/offline_tsm_0118_1"];
        //unzip it into _extractPath
        NSError *errorUnzip;
        if ([SSZipArchive unzipFileAtPath:_zipPath toDestination:_unzipPath overwrite:YES password:nil error:&errorUnzip delegate:nil]) {
            NSLog(@"Extracting Unzip Path = %@",_unzipPath);
        ...
        NSString *_folderName = @"offline_tsm_0118_1";
