...
final class TermsHashPerField extends InvertedDocConsumerPerField {
  ...
  final ByteBlockPoolIndex bytePool;
  ...
  public void initReader(ByteSliceReader reader, int termID, int stream) {
    ...
    reader.init(bytePool,
                postingsArray.byteStarts[termID]+stream*ByteBlockPoolIndex.FIRST_LEVEL_SIZE,
                ints[upto+stream]);
  }
  ...
  public void add(int textStart) throws IOException {
    ...
    if (termID == -1) {
      ...
      if (DocumentsWriter.BYTE_BLOCK_SIZE - bytePool.byteUpto < numPostingInt*ByteBlockPoolIndex.FIRST_LEVEL_SIZE)
        bytePool.nextBuffer();
      ...
      for(int i=0;i<streamCount;i++) {
        final int upto = bytePool.newSlice(ByteBlockPoolIndex.FIRST_LEVEL_SIZE);
